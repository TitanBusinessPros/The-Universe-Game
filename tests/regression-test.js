...
const {
    ...
    UNIT_BUILDING_REQUIREMENTS, PRODUCTION_BUILDING_LABELS, BUILDING_ATTACKER_TYPES,
...
// Direct report (2026-09-08): "I'm on round 12 and nobody has attacked me...
// stop fucking around and find the problem already." Found via live
// simulation against the real deployed site (not tests/balance's own
// artificial tight-ring layout, which can't surface this): nations WERE
// reliably reaching the player - a unit got within single-digit distance of
// the player's own homeworld by turn 5-9 in every sampled run - but
// aiTurn()'s build-type pick used to be a flat, uniform roll across all 19
// buildable types, and 11 of them (deepglider, skyfortress, blazefalcon,
// cargohauler, skyripper, vortexhawk, razorwing, tidebreaker, whisperwind,
// stormbreakerbomber, frostwing) can never damage a building at all (see
// canAttackBuildings()). A nation could amass 10 units, park right on top of
// the player's homeworld, and still land zero attacks purely because none of
// what it built could ever have hit anything - 4 of 5 real 12-turn browser
// runs against the live site showed exactly zero home damage despite
// confirmed close contact.
check("aiTurn()'s build-type pick is weighted toward BUILDING_ATTACKER_TYPES, not a flat uniform roll across every buildable type (root cause of 'nobody has attacked me' despite confirmed contact)", () => {
    const problems = [];
    const island = new Island(0, 0, 0);
    const country = new Country(0, 'BuildWeightTest', '#ff0000', island, false);
    // Every production building standing, tech maxed out, endless resources -
    // isolates this to the type-PICK itself, not some other gate stopping a
    // particular type from ever being chosen.
    country.resources = 1000000;
    TECH_TREE && Object.keys(TECH_TREE).forEach(id => country.researchedTech.add(id));
    gameState.countries = [country];
    vm.runInContext('AI_BUILD_CHANCE = 2;', context); // guarantee attempts every call, no lucky/unlucky roll noise
    vm.runInContext('AI_MOVEMENT_CHANCE = 0; AI_ATTACK_CHANCE = 0;', context); // isolate to the build step alone

    const SAMPLE_TURNS = 200;
    const built = [];
    for (let i = 0; i < SAMPLE_TURNS; i++) {
        country.resources = 1000000; // don't let resources run out across samples
        country.aiTurn();
        built.push(...country.units.map(u => u.type));
        country.units = []; // isolate each sample - don't let unit count itself affect later hunt/build decisions
    }
    ...
    if (built.length < SAMPLE_TURNS * 0.5) problems.push(`expected close to one build per sampled turn with AI_BUILD_CHANCE=2 and ample resources, only got ${built.length} units across ${SAMPLE_TURNS} turns`);
    const attackerShare = built.filter(t => BUILDING_ATTACKER_TYPES.includes(t)).length / built.length;
    // A flat, unweighted 19-type roll would land here around 8/19 ≈ 0.42 at
    // best (and lower once ground troops, which need separate Cargohauler
    // transport, are discounted) - require comfortably above that so this
    // fails loudly if the weighting here is ever quietly reverted or diluted.
    if (attackerShare < 0.55) {
        problems.push(`expected a clear majority of AI builds to be BUILDING_ATTACKER_TYPES, got ${(attackerShare * 100).toFixed(0)}% across ${built.length} sampled units`);
    }
    return problems;
});
...
// ---------- 19b. Map Select (2026-09-12) ----------
//    Country select now opens a "choose your map" screen instead of jumping
//    straight into startGame() - see MAP_CONFIGS' own comment for why every
//    one of the 10 slots is currently the same placeholder.

check('MAP_CONFIGS offers exactly 10 distinct, fully-described map slots', () => {
    const problems = [];
    if (!Array.isArray(MAP_CONFIGS) || MAP_CONFIGS.length !== 10) {
        problems.push(`expected 10 MAP_CONFIGS entries, got ${MAP_CONFIGS && MAP_CONFIGS.length}`);
    }
    const ids = new Set((MAP_CONFIGS || []).map(m => m.id));
    if (ids.size !== (MAP_CONFIGS || []).length) problems.push('MAP_CONFIGS ids are not unique');
    (MAP_CONFIGS || []).forEach(map => {
        if (typeof map.nationCount !== 'number' || map.nationCount <= 0) problems.push(`${map.name}: bad nationCount ${map.nationCount}`);
        if (typeof map.alienPlanetCount !== 'number' || map.alienPlanetCount <= 0) problems.push(`${map.name}: bad alienPlanetCount ${map.alienPlanetCount}`);
        if (!Array.isArray(map.specials) || map.specials.length === 0) problems.push(`${map.name}: no specials listed`);
    });
    return problems;
});

check('openMapSelect()/closeMapSelect() toggle the Single Map setup panel and the map grid together', () => {
    const problems = [];
    document.getElementById('singleMapSetup').style.display = 'block';
    document.getElementById('mapSelectScreen').style.display = 'none';

    openMapSelect(0);
    if (document.getElementById('singleMapSetup').style.display !== 'none') problems.push('openMapSelect() should hide the Single Map setup panel');
    if (document.getElementById('mapSelectScreen').style.display !== 'block') problems.push('openMapSelect() should show the map grid');

    closeMapSelect();
    if (document.getElementById('mapSelectScreen').style.display !== 'none') problems.push('closeMapSelect() should hide the map grid');
    if (document.getElementById('singleMapSetup').style.display !== 'block') problems.push('closeMapSelect() should show the Single Map setup panel again');
    return problems;
});

check('populateMapGrid() renders one clickable card per MAP_CONFIGS entry into #mapGrid', () => {
    const grid = document.getElementById('mapGrid');
    grid.innerHTML = '';
    populateMapGrid();
    const cards = grid.querySelectorAll('.mapCard');
    if (cards.length !== MAP_CONFIGS.length) return [`expected ${MAP_CONFIGS.length} .mapCard elements, found ${cards.length}`];
    return [];
});
...
check('picking a country opens the map screen instead of jumping straight into startGame()', () => {
    ...
    const firstCountryBtn = document.getElementById('countrySelect').querySelector('button');
    firstCountryBtn.onclick();
    if (document.getElementById('mapSelectScreen').style.display !== 'block') problems.push('clicking a country button should open the map screen');
    if (gameState.playerCountry) problems.push('startGame() should not have run yet - the player hasn\'t picked a map');
    return problems;
});

// mapBackgroundImage/mapBackgroundLoaded are `let` bindings that get
// REASSIGNED (a new Image() each time a sector changes) - a destructured
// const captured once at grab-refs time would go stale the moment that
// happens, same reasoning as frameDeltaTime elsewhere in this file. Read
// live instead.
...
check('chooseMap() records the pick, closes the map screen, and finishes starting the game for the pending country', () => {
    gameState.countries = [];
    gameState.playerCountry = null;
    vm.runInContext('initGame();', context, { filename: 'choose-map-setup.js' });
    openMapSelect(2); // country index 2
    chooseMap(5);
    const problems = [];
    ...
    if (currentMapBackgroundSrc() !== MAP_IMAGE_URLS[6]) problems.push(`chooseMap(5) (Sector 6) should swap the actual in-game background to map-6.jpg, got ${currentMapBackgroundSrc()}`);
    return problems;
});
...
// what you see while playing. These check the real fix: a dedicated
// mapBackgroundImage (see its own comment) actually gets set to match.
...
// Real bug found live (not just a hypothetical): spaceBackgroundImage is
// queued for the DEFAULT background at page load via queueImageLoad()'s
// concurrency-limited queue - reassigning that SAME shared object's .src for
// a chosen map meant the queue could dispatch its already-pending default
// load moments later and silently revert it. Proving the fix architecturally:
// applyMapBackground() must never touch spaceBackgroundImage at all.
check('applyMapBackground() never touches spaceBackgroundImage - the default background image is immune to the map-select race that used to revert it', () => {
    const before = spaceBackgroundImage.src;
    applyMapBackground(1);
    applyMapBackground(4);
    applyMapBackground(null);
    return spaceBackgroundImage.src === before ? [] : [`spaceBackgroundImage.src changed from ${before} to ${spaceBackgroundImage.src} - applyMapBackground() should only ever touch the separate mapBackgroundImage`];
});
...
check('startHotSeatGame() resets the background to default so it can\'t inherit a leftover Standard Game map selection', () => {
    ...
    if (currentMapBackgroundSrc()) return [`expected the default background to be restored, got ${currentMapBackgroundSrc()}`];
    return [];
});
...
check('buildSaveData()/applySaveData() round-trip the chosen map so Continue/Load shows the right background, not the default', () => {
    ...
    if (currentMapBackgroundSrc() !== MAP_IMAGE_URLS[7]) return [`expected applySaveData() to restore Sector 7's background, got ${currentMapBackgroundSrc()}`];
    return [];
});
...
check('isGroundUnitInCargo() - true only once a ground unit is actually boarded into a cargohauler, false for a vessel and for a ground unit still standing free (root-cause fix for "troops no longer appear on the minimap when sent anywhere": drawMinimap() had its own unit-dot loop that never shared this check with Unit.draw(), so a boarded trooper kept a dot frozen at its old pickup spot on the minimap for the whole trip)', () => {
    const problems = [];
    const island = new Island(0, 0, 0);
    const country = new Country(0, 'CargoMinimapTest', '#ff0000', island, true);
    gameState.countries = [country];
    gameState.playerCountry = country;

    const cargohauler = new Unit(0, 0, 'cargohauler', 0);
    const freeTrooper = new Unit(100, 100, 'groundpounders', 0);
    const boardedTrooper = new Unit(200, 200, 'groundpounders', 0);
    country.units = [cargohauler, freeTrooper, boardedTrooper];

    if (isGroundUnitInCargo(freeTrooper)) problems.push('expected a ground unit not in any cargohauler to read as NOT in cargo');
    if (isGroundUnitInCargo(cargohauler)) problems.push('expected a non-ground unit (the cargohauler itself) to always read as NOT in cargo');

    cargohauler.cargo.push(boardedTrooper);
    if (!isGroundUnitInCargo(boardedTrooper)) problems.push('expected the boarded trooper to read as in cargo once pushed into a cargohauler\'s cargo array');
    if (isGroundUnitInCargo(freeTrooper)) problems.push('boarding one trooper should not affect a different, still-free trooper');
...
check('startCampaignStage() also resets the map-select background - a Standard Game map choice must not leak into Campaign', () => {
    ...
    if (currentMapBackgroundSrc()) problems.push(`expected the default background restored, got ${currentMapBackgroundSrc()}`);
    return problems;
});
...
// ---------- Landmark/unit screen-visibility (2026-09-12) ----------
// Direct report: "My radar ships on map 2 turned into a red sun looking
// object until I sent a ship close to them and then they turned back into my
// radar ships that were there." Reproduced live: Hotsun's real size (10x a
// planet's) means at a normal zoom, sitting right on top of it, its sprite is
// far bigger than the whole viewport - and separately, Sector 2's own
// background art (a wide, intensely red nebula) is sized to the galaxy's own
// extent, so it too fills the entire screen at a normal zoom. Either way, a
// tiny ~30-unit ship icon drawn on top has almost no contrast to be seen at a
// glance. Two independent fixes, tested here: capping how much screen space
// a landmark's sprite can ever cover, and a dark contrast backing plate
// behind every unit regardless of what's behind it.

check("GalaxyCore.draw() caps how much of the screen it can cover, even sitting right on top of it - a landmark can never completely swallow the viewport", () => {
    const problems = [];
    const core = new GalaxyCore(0, 0, undefined, 999999999); // absurdly large size, sprite never "completes" in this headless test env
    camera.x = 0; camera.y = 0; camera.zoom = 1;
    let capturedRadius = null;
    const origArc = ctx.arc;
    ctx.arc = function (x, y, r, ...rest) { capturedRadius = r; return origArc.apply(this, [x, y, r, ...rest]); };
    core.draw();
    ctx.arc = origArc;
    if (capturedRadius === null) return ['expected GalaxyCore.draw() to call ctx.arc() in its fallback (no-sprite) render path'];
    const maxExpected = Math.min(canvas.width, canvas.height) * GALAXY_LANDMARK_MAX_SCREEN_FRACTION;
    if (capturedRadius > maxExpected + 1) problems.push(`expected the drawn radius capped at ~${maxExpected.toFixed(0)}px regardless of the landmark's real size, got ${capturedRadius.toFixed(0)}px`);
    return problems;
});

check('Unit.draw() draws a dark contrast backing plate behind every unit, before its sprite/glow - keeps ships visible against bright or busy background art', () => {
    const problems = [];
    const island = new Island(0, 0, 0);
    const country = new Country(0, 'ContrastTest', '#fff', island, true);
    gameState.countries = [country];
    gameState.playerCountry = country;
    const unit = makeUnit('radar', 0);
    unit.x = 0; unit.y = 0;
    camera.x = 0; camera.y = 0; camera.zoom = 1;

    const fillStylesAtArcTime = [];
    const origArc = ctx.arc;
    ctx.arc = function (...args) { fillStylesAtArcTime.push(ctx.fillStyle); return origArc.apply(this, args); };
    unit.draw();
    ctx.arc = origArc;

    const hasBlackBacking = fillStylesAtArcTime.some(fs => typeof fs === 'string' && /rgba\(\s*0,\s*0,\s*0,/.test(fs));
    if (!hasBlackBacking) problems.push(`expected at least one dark (black rgba) backing arc drawn behind the unit, got fillStyles at each arc() call: ${JSON.stringify(fillStylesAtArcTime)}`);
    return problems;
});
